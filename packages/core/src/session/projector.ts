...
const projectFork = Effect.fn("SessionProjector.projectFork")(function* (
  ...
  const stored = yield* db
    ...
    .values({
      ...
      workspace_id: event.data.location?.workspaceID ?? parent.workspace_id,
      slug: Slug.create(),
      directory: event.data.location?.directory ?? parent.directory,
      path: event.data.location ? event.data.subpath : parent.path,
